...
for(h in 1:nreps) {
  ...
  tmp_cf <- regression_forest(X = Xmat_tmp,
                              Y = Y)

  # Reduced model for Causal Forest
  f_cf <- function(tau) {
    Wtau <- Y - tau * A
    fit_reduced <- regression_forest(X = X0mat_notrt_tmp, Y = Wtau, num.trees = 100)
    pred_reduced <- predict(fit_reduced, X0mat_notrt_tmp)$predictions
    mse_local <- mean((Wtau - pred_reduced)^2)
    return(mse_local)
  }
  tau.star.cf <- optimize(f_cf, interval=c(-5, 5))$minimum
  Wtau.star.cf <- Y - tau.star.cf * A
  tmp_reduced_cf <- regression_forest(X = X0mat_notrt_tmp, Y = Wtau.star.cf, num.trees = 100)
  ...
  # rlearner model
  #tmp_rlearner <- rlasso(x=X_matrix, w=A, y=Y)
...
#  f_rlearner <- function(tau) {
 ...
 # tmp_reduced_rlearner <- lm(Wtau.star.rlearner ~ ., data = data.frame(X_matrix))
  ...
  pp_full_cf <- predict(tmp_cf, Xmat_tmpk)$predictions
  pp_reduced_cf <- predict(tmp_reduced_cf, X0mat_notrt_tmpk)$predictions
 ...
 # pp_full_bartcause <- baseline_bc + ate_estimate * Ak
  ...
  ncv_cf <- nested_cv(X=XX, X0=XX0, Y=as.vector(Y), Trt=A, tau.range=tau.range, funcs=causal_forest_funs,
                      n_folds = n_folds, reps = 10, n_cores=4)
