...
for(j in 1:length(muvec)) {
  ...
  results <- parLapply(cl, rep(5000, 500), function(n) { OneSimTwoRun(n, mu_choice=muvec[j], theta_choice=thetavec[j], p=9, n_folds=5, nested_cv_reps=50, ranfor=rfrun) } )
  ...
  fname <- paste("~/Projects/HTE-Model-Comparison/Simulation2/Result", muvec[j],"Theta",thetavec[j],"n5000.RData", sep="")
