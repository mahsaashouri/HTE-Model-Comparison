...
OneSimTwoRun <- function(n, mu_choice, theta_choice, p, n_folds, nested_cv_reps, ranfor=TRUE) {
  ...
  for(h in 1:nreps) {
    ...
    if(ranfor) {
      ...
      true_thetas[h,] <- c(theta_lm, theta_glmnet, theta_glmboost, NA)
    }
