...
naive_cv <- function(X, X0, Y, Trt, tau.seq, funcs, n_folds, alpha = c(0.01, 0.05, 0.1, 0.25, 0.5),
  ...
  for(k in 1:n_folds) {
    ...
    Y_hat <- funcs$predictor(fit$full, X[fold_id == k, ], Trt[fold_id == k], funcs_params = funcs_params)
    Y_hat_reduced <- funcs$predictor(fit$reduced, X0[fold_id == k, ], Trt[fold_id == k], funcs_params = funcs_params)
...
nested_cv_helper <- function(X, X0, Y, Trt, tau.seq, funcs, n_folds, funcs_params = NULL) {
  ...
  for(f1 in 1:(n_folds - 1)) {
    for(f2 in (f1+1):n_folds) {
      ...
      preds <- funcs$predictor(fit$full, X, Trt, funcs_params = funcs_params)
      preds_reduced <- funcs$predictor(fit$reduced, X0, Trt, funcs_params = funcs_params)
  ...
  for(f1 in 1:(n_folds)) {
    ...
    fit <- funcs$fitter(as.matrix(X[-test_idx, ]), as.matrix(X0[-test_idx, ]), Y[-test_idx], Trt[-test_idx], tau.seq, funcs_params = funcs_params)
    preds <- funcs$predictor(fit$full, X[test_idx, ], Trt[test_idx], funcs_params = funcs_params)
    preds_reduced <- funcs$predictor(fit$reduced, X0[test_idx, ], Trt[test_idx],funcs_params = funcs_params)
